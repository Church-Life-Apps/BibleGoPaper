...
Figure~\ref{fig:tiers} shows the distribution of tier-weighted semantic similarity across the three commentator tiers, expressed as the percentage of each tier's contribution to the final BHT's similarity profile (values sum to 100\% per verse, averaged across the corpus). Both testaments show a flatter distribution than the prompt's ``primarily Tier~1, sparingly Tier~2, minimally Tier~3'' instruction would suggest. NT: Tier~1 31.8\%, Tier~2 31.2\%, Tier~3 37.0\%. OT: Tier~1 35.1\%, Tier~2 29.7\%, Tier~3 35.1\%. In both corpora the Tier~3 commentators (Calvin, Gill, Wesley, and others) contribute at or above the level of Tier~1, indicating that the tier instruction is not strongly followed in deployed output. One likely cause is that several Tier~3 commentators produce longer and more thematically broad excerpts than the Tier~1 commentators, yielding higher surface-level similarity scores even when they contribute less content by intent. Reconciling the intent-based tiering with similarity-based measurement is a natural follow-up for future tuning.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{figures/tier_breakdown.pdf}
\caption{Average commentator-tier contribution to BHT semantic similarity, as a percentage summing to 100\% per verse. Tier~3 contributes at or above the level of Tier~1 in both corpora, indicating the tier instruction is not strongly followed in deployed output.}
\label{fig:tiers}
\end{figure}

\subsection{Commentary-Accuracy Score Distribution}

Figure~\ref{fig:quality_dist} presents the distribution of commentary-accuracy scores across the NT and OT corpora. Both distributions are approximately Gaussian with a right-skew tail toward higher-scoring verses. For the NT corpus ($n{=}7{,}957$), the 5th, 25th, 50th, 75th, and 95th percentiles are 1.32, 1.68, 1.93, 2.14, and 2.46 respectively. The combined OT corpus ($n{=}35{,}798$ across regular and split pipelines) runs slightly lower: 1.29, 1.58, 1.80, 2.02, and 2.39 at the same percentiles. The minimum-acceptance threshold in the hard-constraint gate is 2.3 (Section~\ref{sec:methods}, Phase~5), which sits near the 85th percentile of deployed scores---a deliberately permissive threshold reflecting the best-of-five fallback policy: if no attempt clears~2.3, the highest-scoring attempt is still served to avoid coverage gaps. A meaningful minority of served BHTs therefore fall below the nominal threshold, a known trade-off between full-Bible coverage and score strictness.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{figures/quality_distribution.pdf}
\caption{Distribution of commentary-accuracy scores across the NT ($n{=}7{,}957$) and OT ($n{=}35{,}798$, regular and split pipelines combined) corpora. Both distributions are approximately Gaussian with a right-skew tail; the nominal acceptance threshold (2.3) sits near the 85th percentile of deployed scores.}
