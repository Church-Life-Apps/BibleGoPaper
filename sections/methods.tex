...
% Lane headers
\node[lanehdr, color=tier1] (nthdr) at (-3.2, 0) {NT route};
\node[lanehdr, color=tier3] (othdr) at ( 3.2, 0) {OT route};
% Lane inputs
...
\node[flowstep, right=1cm of d1, fill=tier3!15, draw=tier3, text width=3.0cm] (retry) {\textbf{Retry with feedback}\\\scriptsize up to 5 attempts\\\scriptsize (prior BHT + specific fixes)};
