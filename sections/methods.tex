...
\begin{tikzpicture}[
    ...
    flowstep/.style={rectangle, draw=pipeline, fill=pipeline!8, text width=3.1cm, minimum height=0.6cm, align=center, font=\scriptsize\sffamily, rounded corners=2pt, line width=0.5pt},
    ntstep/.style={rectangle, draw=tier1, fill=tier1!10, text width=3.1cm, minimum height=0.6cm, align=center, font=\scriptsize\sffamily, rounded corners=2pt, line width=0.5pt},
    otstep/.style={rectangle, draw=tier3, fill=tier3!10, text width=3.1cm, minimum height=0.6cm, align=center, font=\scriptsize\sffamily, rounded corners=2pt, line width=0.5pt},
...
\node[ntstep, below left=0.55cm and 0.1cm of load] (nt) {\textbf{NT route}\\verse-level commentary};
\node[otstep, below right=0.55cm and 0.1cm of load] (ot) {\textbf{OT route}\\multi-verse commentary};
% OT has retrieval step first
\node[otstep, below=0.35cm of ot] (retrieval) {Retrieve relevant\\chunks (vector search)};
% Both feed into quote extraction
\node[flowstep, below=1.15cm of load] (qe) {Quote extraction\\(3--5 quotes per commentator)};
% Common tail
\node[flowstep, below=of qe] (group) {Group quotes by\\Tier 1 / 2 / 3};
\node[flowstep, below=of group] (gen) {BHT generation\\(regular or split variant)};
\node[decision, below=0.45cm of gen] (d1) {Passes gate\\\& score $\geq 2.3$?};
\node[flowstep, right=0.7cm of d1, fill=tier3!15, draw=tier3] (retry) {Retry\\(up to 5$\times$)};
\node[flowstep, below=0.45cm of d1] (best) {Return best-scoring\\candidate};
